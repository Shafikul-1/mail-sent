...
@section('othersContent')
    <form method="POST" action="{{route('authUser')}}" class="max-w-sm mx-auto mb-6">
        @csrf
            <div class="mb-5">
    ...
    <div class="flex items-center justify-center w-full h-full button dark:bg-white">
        <a href="{{route('user.create')}}" class="rounded-md px-3.5 py-2 m-1 overflow-hidden relative group cursor-pointer border-2 font-medium border-indigo-600 text-indigo-600 text-white">
        <span class="absolute w-64 h-0 transition-all duration-300 origin-center rotate-45 -translate-x-20 bg-indigo-600 top-1/2 group-hover:h-64 group-hover:-translate-y-32 ease"></span>
        <span class="relative text-indigo-600 transition duration-300 group-hover:text-white ease">Signup</span>
        </a>
    </div>
