...
<form action="{{ route('composeSent') }}"  method="post" enctype="multipart/form-data" id="composeSubmit" class="max-w-sm mx-auto">
    @csrf
    <div class="mb-5">
      ...
      @error('to')  <p class="text-red-800">{{$message}}</p> @enderror
    </div>
    <div class="mb-5">
      ...
      @error('subject')  <p class="text-red-800">{{$message}}</p> @enderror
    </div>
    <div class="mb-5">
    ...
    <div class="mb-5">
        <label for="sendingTime" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Your message</label>
        <input type="text" value="{{old('sendingTime')}}" name="sendingTime" id="sendingTime"  class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Sending Time here ..." />
